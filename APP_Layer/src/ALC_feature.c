...
app_status_t ALC_Task(void)
{
    ...
    if (!FlagtoChange)
    {
        ...
        {
            /* go right */
            l_AppStatus |= LaneChange(RIGHT);
            FlagtoChange = 1;
        }
        else if (CurrentPosition == DRIFT_RIGHT)
        {
            /* go left */
            l_AppStatus |= LaneChange(LEFT);
            FlagtoChange = 1;
        }
        else if (WantedLaneToChange == LEFT)
        {
            /* go left */
            l_AppStatus |= LaneChange(LEFT);
            FlagtoChange = 1;
        }
        else if (WantedLaneToChange == RIGHT)
        {
            /* go right */
            l_AppStatus |= LaneChange(RIGHT);
            FlagtoChange = 1;
    ...
    {
        if (CurrentPosition == CENTERALIZED)
        {
            /* stay at this lane */
...
{
    app_status_t l_AppStatus = APP_OK;
    if (((AvailableLanes == LEFT_ONLY) || (AvailableLanes == LEFT_AND_RIGHT)) && (p_WantedLane == LEFT))
    {
        WantedLaneToChange = LEFT;
    }
    else if (((AvailableLanes == RIGHT_ONLY) || (AvailableLanes == LEFT_AND_RIGHT)) && (p_WantedLane == RIGHT))
    {
        WantedLaneToChange = RIGHT;
    }
    else
    {
